...
fn_wallcache()
{
    ...
    [ ! -e "${thmbDir}/${x_hash}.quad" ] && magick "${thmbDir}/${x_hash}.sqre" \
        \( -size 500x500 xc:white -fill \"rgba(0,0,0,0.7)\" -draw \"polygon 400,500 500,500 500,0 450,0\" -fill black -draw \"polygon 500,500 500,0 450,500\" \) -alpha Off -compose CopyOpacity -composite "${thmbDir}/${x_hash}.png" && mv "${thmbDir}/${x_hash}.png" "${thmbDir}/${x_hash}.quad"
